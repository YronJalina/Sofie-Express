// FLIGHT DATA
let flights = [
  { date: "2026-02-24", route: "Manila → Dubai", capacity: 46, totalCapacity: 46 },
  { date: "2026-02-26", route: "Manila → Dubai", capacity: 115, totalCapacity: 115 }
];
...
function renderFlights(){
  ...
  flights.forEach((flight)=>{
    ...
    const isNextFlight = flightDate >= now && (!flights.some(f=>new Date(f.date)>=now && new Date(f.date)<flightDate));
    const card=document.createElement('div');
    card.className='flight-card';
    if(isNextFlight) card.classList.add('next-flight');
    const progressPercent=Math.floor((flight.capacity/flight.totalCapacity)*100);
    card.innerHTML=`${isNextFlight?'<div class="badge">Next Flight</div>':''}
      <h3>${flight.route}</h3>
      <p>Date: ${flight.date}</p>
      <div class="progress-bar"><div class="progress-fill" style="width:${progressPercent}%"></div></div>
      <p><strong>${flight.capacity} KG Available</strong></p>`;
...
document.querySelectorAll('a, button').forEach(el=>{
  el.addEventListener('mouseenter',()=>{cursor.style.transform='translate(-50%, -50%) scale(2)';cursor.style.backgroundColor='rgba(255,122,0,0.4)';});
  el.addEventListener('mouseleave',()=>{cursor.style.transform='translate(-50%, -50%) scale(1)';cursor.style.backgroundColor='rgba(255,122,0,0.8)';});
});
